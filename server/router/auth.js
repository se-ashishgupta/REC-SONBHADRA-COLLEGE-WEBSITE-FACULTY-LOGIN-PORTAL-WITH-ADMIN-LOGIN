...
router.delete("/deleteaccount/:id", async (req, res) => {
  try {
    ...
    const uid1 = await User.findOne({ email: data });
    if (!uid && !uid1) {
      return res.status(401).send("Account Not found");
    } else {
      const uidd = await User.deleteOne({ userid: data });
      const uidd1 = await User.deleteOne({ email: data });
      if (uidd || uidd1) {
...
// RS DATA
router.get("/getrsdatatowebsite", async (req, res) => {
  console.log("hello from inside data to user website");
  try {
    const userid = await User.find({
      position: "Research Schollar",
    });
    res.status(200).send(userid);
  } catch (err) {
    res.status(400).send(err);
...
// Technical Staffs DATA
router.get("/gettsdatatowebsite", async (req, res) => {
  console.log("hello from inside data to user website");
  try {
    const userid = await TechnicalStaffUser.find();
    res.status(200).send(userid);
  } catch (err) {
    res.status(400).send(err);
  }
...
router.delete("/deletetechnicalstaff/:id", async (req, res) => {
  try {
    ...
    const uid1 = await TechnicalStaffUser.findOne({ email: data });
    if (!uid && !uid1) {
      return res.status(401).send("Account Not found");
    } else {
      const uidd = await TechnicalStaffUser.deleteOne({ userid: data });
      const uidd1 = await TechnicalStaffUser.deleteOne({ email: data });
      if (uidd || uidd1) {
        return res.status(201).send("Account Deleted");
      }
    }
